...
$quit_time = '5:00:00pm'  # Modify quit_time for when you want the program to stop
...
while ($true) {
  ...
  if ($RNDSleep % 5 -eq 0) {
     ...
     $counter = 1
     $COUNTERRandom = Get-Random -minimum 8 -maximum 20
     while ($counter -lt $COUNTERRandom) {
        $RNDSleep = Get-Random -minimum 927 -maximum 4091
  ...
  $RNDSleep = Get-Random -minimum 727 -maximum 16001
  Write-Output $RNDSleep
  Start-Sleep -milliseconds $RNDSleep
